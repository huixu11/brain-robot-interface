...
（分屏提示：为了录 15-30s 的高质量 demo 视频/GIF，可以加 `--telemetry` 打开一个小窗（优先 OpenCV；若 `cv2` 不可用则自动回退到 Tkinter），显示 `p_move(t)` 曲线、cue window、当前 action，右侧终端不需要再刷大量日志。录屏时把 MuJoCo viewer 和 telemetry 小窗并排即可。）
